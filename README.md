...
6. 选择 `快览`、`标准` 或 `深度`，点击“开始总结”。
7. 完成后，离线 HTML 学习页会自动打开。

标准 `/video/BV...` 地址、`b23.tv` 短链和带 `bvid` 的 `/list/watchlater/` 稍后再看地址都可以直接粘贴。稍后再看地址会先转换成标准单视频地址，列表配置、`oid` 与追踪参数不会交给下载器。
...
数学公式会在生成 HTML 时静态转换为 MathML，断网可读，也不依赖 CDN 或页面脚本。展示公式支持 `\[...\]` 与 `$$...$$`，行内公式支持 `\(...\)` 与 `$...$`；代码行和代码块里的同类符号保持原样。
...
未设置 `CODEX_HOME` 时，skill 会同步到当前用户目录下的 `.codex`。仓库和运行数据可以放在非系统盘，Codex 的登录凭据仍由 Codex CLI 自己管理。自动化场景也可使用 `BILI_NOTES_TEXT_PROVIDER`、`BILI_NOTES_TEXT_MODEL`、`BILI_NOTES_TEXT_BASE_URL` 和各厂商标准 API Key 环境变量覆盖界面设置。
