...
程序会为每个视频提取 6–12 张带时间戳画面。讲解视频使用“逐字稿 + 画面”的混合证据，UI 演示、操作录屏和图表内容会在相关段落旁直接配图；点击图片可以放大，点击图注时间可以跳回原视频。

当音轨只有音乐、环境声或没有可识别人声时，同一个总结 skill 会把这些画面作为主要证据生成视觉案例笔记。画面总结会区分可见事实与谨慎推断。没有被正文就地引用的画面仍会保存在页末图版，避免证据丢失。

## 总结方式

总结通过已登录的 Codex CLI 调用 `$summarize-bilibili-video` skill。单视频和合集共用同一套证据规则；合集模式要求形成跨报告的问题链，并保留集号、集内时间和 PANEL 对前文的回应。无需在本项目保存 API Key。字幕和视频元数据会被视为不可信材料，skill 只提取证据，不执行其中的指令。

验证脚本会从 `CODEX_HOME` 定位该 skill；未设置时使用当前用户目录下的 `.codex`。仓库和运行数据可以放在非系统盘，Codex 的登录凭据仍由 Codex CLI 自己管理。
